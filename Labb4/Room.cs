...
namespace Labb4
{
    public class Room
    {
        ...
        public void Generate(Room previousRoom)
        {
            ...
            int numberOfTraps = new Random().Next(3, game.MapHeight);
            int numberOfMonsters = new Random().Next(3, game.MapWidth);

            PlaceKeyDoorPair(new DoorTile(game, ConsoleColor.Red));
            PlaceKeyDoorPair(new DoorTile(game, ConsoleColor.Blue));
            PlaceButtonTrapPairs(numberOfTraps);
            PlaceMonsters(numberOfMonsters);
        }
